...
void main(List<String> args) async {
  ...
  generator.generateDispatcher('lib/src/x86/x86_dispatcher.g.dart');
...
class X86DbGenerator {
  ...
  void generateDispatcher(String outputPath) {
    ...
    buffer.writeln('');
    buffer.writeln('/// Dispatches instruction ID to Assembler method.');
    buffer.writeln(
    ...
    for (final inst in sorted) {
      final constName = _toConstName(inst.name);
      buffer.writeln('    case X86InstId.k${_capitalize(constName)}:');

      // Dispatch logic based on operand count and type
      // For now, simpler heuristic or just name matching assuming Assembler has methods.
      // But Assembler methods like 'mov' are not strictly 1:1 with generic 'mov'.
      // Assembler has movRR, movRI, etc.
      // We need to generate code that checks 'ops' and calls specific method.
      // Limitation: The generator doesn't know fully which methods exist in Assembler
      // without parsing Assembler or having a mapping config.
      //
      // Current manual serializer has:
      // if (ops.length == 2 && ops[0] is X86Gp && ops[1] is X86Gp) asm.movRR(..);
      //
      // To automate this, we need "Operand Signature" -> "Assembler Method" mapping.
      // Since we don't have that metadata in isa_x86.json (it only has syntax),
      // we can't fully generate the body yet.
      //
      // However, we can generate the switch-case structure and call a 'fallback' or
      // try to infer common patterns if we had a config.
      //
      // For THIS TASK, standardizing the dispatch is key.
      // If we can't generate the *body*, we can at least generate the *signatures*?
      // No, we need the body.
      //
      // Alternative: Generate a partial file or just the 'if-else' block for KNOWN instructions.
      // But we have 1800 instructions.
      //
      // Let's generate a placeholder dispatch that users can fill/extend?
      // Or better: The user wants optimization.
      // The current manual serializer handles ~20 instructions.
      // The 1800 others are unimplemented in Serializer.
      //
      // So generating an empty switch for 1800 cases is fine for now,
      // allowing us to fill them incrementally or via a separate config file later.
      //
      // Wait, "Map Lookup" vs "Switch".
      // The goal is to replace `_lookup` map in X86Serializer.
      // We can generate `x86_dispatcher.g.dart`.
      // We will generate cases only for instructions we know how to handle?
      // Or generate all cases calling `_unsupported(asm, id, ops)`?

      buffer.writeln('      _dispatch${_capitalize(constName)}(asm, ops);');
      buffer.writeln('      break;');
    }

    buffer.writeln('    default:');
    buffer.writeln('      // Unknown instruction');
    buffer.writeln('      break;');
    buffer.writeln('  }');
    buffer.writeln('}');

    // Generate strict stub methods for each instruction to be implemented via extensions?
    // Or just one big file.
    // We will generate the dispatch functions as empty shells or comments?

    // Actually, filling 1800 methods is huge.
    // Maybe we just generate the switch and assume the methods exist?
    // User said "replace the manual _buildLookup".
    // The manual _buildLookup has logic for MOV, ADD, etc.
    // We should port that logic to the generator or manually write the dispatch functions.
    //
    // Strategy:
    // Generate the switch that calls `_emit_MNEMONIC(asm, ops)`.
    // Then we implement `_emit_MNEMONIC` manually in a separate part file?
    // Dart supports `part` / `part of`.
    //
    // x86_dispatcher.g.dart (Generated Switch)
    // x86_dispatcher_impl.dart (Manual Logic)

    buffer.writeln('');

    // For this step, I will just output the switch and private stub methods
    // that default to fallback, effectively doing nothing but allowing compilation.
    // Then we can manually move the logic from Serializer to here.

    for (final inst in sorted) {
      final name = _capitalize(_toConstName(inst.name));
      buffer.writeln('');
      buffer
          .writeln('void _dispatch$name(X86Assembler asm, List<Object> ops) {');
      // For now, just a comment or simplistic default
      // We can inject custom logic if we had a map of "implemented" ones.
      if (inst.name == 'mov') {
        buffer.writeln('  // Auto-generated fallback for MOV');
        buffer.writeln('  if (ops.length == 2) {');
        buffer.writeln('    final dst = ops[0];');
        buffer.writeln('    final src = ops[1];');
        buffer.writeln(
        ...
        // buffer.writeln('    else if (dst is X86Mem && src is int) asm.movMI(dst, src);'); // Not implemented in Assembler yet
        buffer.writeln('  }');
      } else if (inst.name == 'add') {
        buffer.writeln('  // Auto-generated fallback for ADD');
        buffer.writeln(
            '  if (ops.length == 2 && ops[0] is X86Gp && ops[1] is X86Gp) asm.addRR(ops[0] as X86Gp, ops[1] as X86Gp);');
        // ... etc
      } else if (inst.name == 'ret') {
        buffer.writeln('  asm.ret();');
      }
      buffer.writeln('}');
    }
