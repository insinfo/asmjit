param(
  [string]$OutDir = ".",
  [string]$OutName = "chacha20_impl"
)

$ErrorActionPreference = "Stop"

$src = Join-Path $PSScriptRoot "chacha20_impl.c"
...
if (Test-Path $clang) {
  try {
    Invoke-Tool $clang @("-O3", "-march=native", "-shared", "-fuse-ld=lld", "-o", $outPath, $src)
    Write-Host "Build successful with Clang: $outPath"
    exit 0
  } catch {
...
if (Test-Path $gcc) {
  try {
    Invoke-Tool $gcc @("-O3", "-march=native", "-shared", "-o", $outPath, $src)
    Write-Host "Build successful with GCC: $outPath"
    exit 0
  } catch {
...
if ((Test-Path $cl) -and (Test-Path $vcvars)) {
  $cmd = "`"$vcvars`" && `"$cl`" /O2 /arch:AVX2 /LD /Fe$outPath `"$src`""
  Write-Host "Running: cmd /c $cmd"
  cmd /c $cmd
  if ($LASTEXITCODE -ne 0) {
    throw "MSVC build failed with exit code $LASTEXITCODE"
  }
  Write-Host "Build successful with MSVC: $outPath"
  exit 0
}
